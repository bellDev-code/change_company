# 블랙잭 - 모든 경우의 수 구하기

# 카지노에서 제일 인기 있는 게임 블랙잭의 규칙은 상당히 쉽다. 카드의 합이 21을 넘지 않는 한도 내에서, 
# 카드의 합을 최대한 크게 만드는 게임이다. 블랙잭은 카지노마다 다양한 규정이 있다.
# 한국 최고의 블랙잭 고수 김정인은 새로운 블랙잭 규칙을 만들어 상근, 창영이와 게임하려고 한다.
# 김정인 버전의 블랙잭에서 각 카드에는 양의 정수가 쓰여 있다. 그 다음, 딜러는 N장의 카드를 모두 숫자가 보이도록 바닥에 놓는다. 
# 그런 후에 딜러는 숫자 M을 크게 외친다.
# 이제 플레이어는 제한된 시간 안에 N장의 카드 중에서 3장의 카드를 골라야 한다. 블랙잭 변형 게임이기 때문에, 
# 플레이어가 고른 카드의 합은 M을 넘지 않으면서 M과 최대한 가깝게 만들어야 한다.
# N장의 카드에 써져 있는 숫자가 주어졌을 때, M을 넘지 않으면서 M에 최대한 가까운 카드 3장의 합을 구해 출력하시오.

# 예제 입력
# 5 21
# 5 6 7 8 9

# 예제 출력
# 21
N, M = map(int, input().split())
card_num = list(map(int, input().split()))

result = 0
for i in range(N):
    for j in range(i + 1, N):
        for z in range(j + 1, N):
            blackjack = card_num[i] + card_num[j] + card_num[z]
            if blackjack <= M:
                result = max(result, blackjack)
print(result)

# 16줄 : N, M의 입력값 설정
# 17줄 : 5, 6, 7, 8, 9 입력값을 []처리 하여 card_num 변수에 넣기
# 19줄 : 결과값 정수 초기화
# 20줄-22줄 : N개의 카드 중 하나를 선택하기 위해 3장을 선택하기 위해 삼중 for문 사용
# i = 0, 1, 2, 3, 4
# 23줄 : 선택한 3장의 카드 blackjack 변수에 넣는다.
# 24줄-25줄 : 합한 값이 M이하라면 result에 현재의 합과 기존의 결과 값 중 최댓값을 저장 


# 이런식으로 전부 찾기 위해서
# i=0, j=1, z=2
# i=0, j=1, z=3
# i=0, j=1, z=4
# i=0, j=2, z=3
# i=0, j=2, z=4
# i=0, j=3, z=4
# ...